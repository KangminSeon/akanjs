...
export class CloudRunner extends runner("cloud") {
  ...
  async #getRemoteEnvServerWithUsername(): Promise<SelectedRemoteEnvServer> {
    ...
    const username = (await input({ message: `SSH username for ${remoteServer.config.host} (optional): ` })).trim();
    return {
      ...remoteServer,
      config: {
        ...remoteServer.config,
  ...
  async downloadEnv(cloudApi: CloudApi, workspace: Workspace, workspaceId: string) {
    ...
    await cloudApi.downloadEnv(workspaceId, path.join(workspace.workspaceRoot, envArchivePath));
