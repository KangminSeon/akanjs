...
export class CloudRunner extends runner("cloud") {
  ...
  async #addRemoteEnvServer(): Promise<SelectedRemoteEnvServer> {
    const name = (
      await input({
        ...
        validate: (value) =>
          value.trim() ? true : "Remote server name is required",
      })
    ).trim();
    const host = (
      await input({
        ...
        validate: (value) =>
          value.trim() ? true : "Remote server host is required",
      })
    ).trim();
    const username =
      (
        await input({ message: "Remote server username (optional): " })
      ).trim() || undefined;
    const portInput = (
      await input({
  ...
  async #selectRemoteEnvServer(): Promise<SelectedRemoteEnvServer> {
    ...
    const serverEntries = Object.entries(servers).sort(([nameA], [nameB]) =>
      nameA.localeCompare(nameB),
    );
    if (serverEntries.length === 0) {
      Logger.info(
        "No remote env servers configured. Add the first remote server for SCP mode.",
      );
  ...
  async #removeRemoteEnvServer(
    serverEntries: [string, RemoteEnvServerConfig][],
  ) {
    const selectedName = await select<string>({
  ...
  async downloadEnv(
    ...
    await cloudApi.downloadEnv(
      workspaceId,
      path.join(workspace.workspaceRoot, envArchivePath),
    );
  ...
  async downloadEnvByScp(workspace: Workspace) {
    ...
    const remoteTarget = this.#getScpTarget(
      remoteServer.config,
      remoteArchivePath,
    );
    await workspace.mkdir("local");
    ...
    try {
      Logger.info(
        `Downloading env archive from remote server "${remoteServer.name}"...`,
      );
      await workspace.spawn(
        "scp",
        this.#getScpArgs(remoteServer.config, remoteTarget, envArchivePath),
        {
          cwd: workspace.workspaceRoot,
          stdio: "inherit",
        },
      ...
      throw new Error(
        `Failed to download env archive from remote server "${remoteServer.name}"`,
        { cause: error },
      );
    }
  }
  async uploadEnvByScp(workspace: Workspace, filePath: string) {
    ...
    try {
      await workspace.spawn(
        "ssh",
        this.#getSshArgs(remoteServer.config, `mkdir -p ${remoteArchiveDir}`),
        {
          cwd: workspace.workspaceRoot,
          stdio: "inherit",
        },
      ...
      Logger.info(
        `Uploading env archive to remote server "${remoteServer.name}"...`,
      );
      await workspace.spawn(
        "scp",
        this.#getScpArgs(remoteServer.config, filePath, remoteTarget),
        {
          cwd: workspace.workspaceRoot,
          stdio: "inherit",
        },
      ...
      throw new Error(
        `Failed to upload env archive to remote server "${remoteServer.name}"`,
        { cause: error },
      );
    }
  }
