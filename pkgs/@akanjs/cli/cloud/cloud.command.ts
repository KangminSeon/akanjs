...
const resolveRegistryUrl = (registry: "npm" | "local") =>
  registry === "local" ? localRegistryUrl() : undefined;
...
export class CloudCommand extends command(
  ...
  ({ public: target }) => ({
    ...
    downloadEnv: target({ desc: "Download environment variables from cloud" })
      .with(Workspace)
      .exec(async function (workspace) {
        await this.cloudScript.downloadEnv(workspace);
      }),
    uploadEnv: target({ desc: "Upload environment variables to cloud" })
      .with(Workspace)
      .exec(async function (workspace) {
