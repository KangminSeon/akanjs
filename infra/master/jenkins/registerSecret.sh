...
JENKINS_USER_PASS="${2:-${JENKINS_USER_PASS:-}}"

if [ -z "$JENKINS_URL" ] || [ -z "$JENKINS_USER" ] || [ -z "$JENKINS_USER_PASS" ]; then
    echo "Missing Jenkins credentials. Copy infra/master/jenkins/credentials.example.sh to infra/master/jenkins/credentials.sh and fill it in, or pass user/password as arguments." >&2
    exit 1
fi
...
SECRET_MAP=$(infra/master/jenkins/getSecrets.sh)
