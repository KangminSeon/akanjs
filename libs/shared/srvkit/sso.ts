...
export const initSsoProviders = (host: string, ssoOptions: SSOOptions) => {
  const origin = host === "localhost" ? "http://localhost:8282/backend" : `https://${host}/backend`;
  if (ssoOptions.kakao) {
  ...
  if (ssoOptions.naver) {
    ssoRegistry.set("naver", {
      credential: ssoOptions.naver,
      callbackURL: `${origin}/user/naver/callback`,
    });
  }
  if (ssoOptions.github) {
    ssoRegistry.set("github", {
      credential: ssoOptions.github,
      callbackURL: `${origin}/user/github/callback`,
      scope: ["user"],
    });
  }
  if (ssoOptions.google) {
    ssoRegistry.set("google", {
      ...
      callbackURL: `${origin}/user/google/callback`,
      scope: ["email", "profile"],
    });
  }
...
export const getOAuthRedirectUrl = (type: Exclude<utilOption.SSOType, "apple">, state?: string): string => {
...
const exchangeCodeForToken = async (type: Exclude<utilOption.SSOType, "apple">, code: string): Promise<string> => {
  const config = ssoRegistry.get(type);
  if (!config) throw new Error(`${type} SSO not configured`);
  const endpoints = oauthEndpoints[type];
...
export const extractKakaoProfile = async (code: string): Promise<KakaoResponse> => {
  const accessToken = await exchangeCodeForToken("kakao", code);
...
export const extractNaverProfile = async (code: string): Promise<NaverResponse> => {
  const accessToken = await exchangeCodeForToken("naver", code);
...
export const extractGithubProfile = async (code: string): Promise<GithubResponse> => {
...
export const extractGoogleProfile = async (code: string): Promise<GoogleResponse> => {
...
export const extractFacebookProfile = async (code: string): Promise<FacebookResponse> => {
